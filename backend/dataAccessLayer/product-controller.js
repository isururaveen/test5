    ...
    db.query(query + productCountQuery, [1, 2], (err, result) => {
      ...
      db.query(query, (err, result) => {
        ...
        resultSet.Products.forEach((element, index) => {
          ...
          resultSet.Products[index]["Size"] = aaa.filter(
            (a) => a.AttributeId == 1
          );
          resultSet.Products[index]["Color"] = aaa.filter(
            (a) => a.AttributeId == 2
          );
        });
...
const GetProductAttributes = (request, response) => {
  try {
    let query =
      "CALL catalog_get_attribute_values(1);CALL catalog_get_attribute_values(2)";
    // execute query
...
const GetFilteredProducts = (request, response) => {
  try {
    ...
    let filterDepartment =
      request.body.paging.DepartmentId == 0
        ? "AND C.department_id = C.department_id"
        : `AND C.department_id = ${request.body.paging.DepartmentId}`;
    //pagination Category
    let filterCategory =
    ...
    if (request.body.paging.SearchString == "") {
      ...
      let likeQuery = [];
      //lopping word from product
      words.forEach((element) => {
        likeQuery.push(
          `P.name like '%${element}%' OR P.description like '%${element}%'`
        );
      });
    ...
    db.query(query + productCountQuery, [1, 2], (err, result) => {
      ...
      db.query(query, (err, result) => {
        ...
        resultSet.Products.forEach((element, index) => {
          var aaa = result.filter((a) => a.ProductId == element.ProductId);
          resultSet.Products[index]["Size"] = aaa
            .filter((a) => a.AttributeId == 1)
            .sort(function (a, b) {
              return a.AttributeValueId - b.AttributeValueId;
            });
          resultSet.Products[index]["Color"] = aaa
            .filter((a) => a.AttributeId == 2)
            .sort(function (a, b) {
              return a.AttributeValueId - b.AttributeValueId;
            });
        });
  ...
  try {
    ...
    db.query(query, (err, result) => {
      ...
      db.query(subquery, (err, results) => {
        ...
        productDetails["Size"] = results
          .filter((a) => a.AttributeId == 1)
          .sort(function (a, b) {
            return a.AttributeValueId - b.AttributeValueId;
          });
        productDetails["Color"] = results
          .filter((a) => a.AttributeId == 2)
          .sort(function (a, b) {
            return a.AttributeValueId - b.AttributeValueId;
          });
